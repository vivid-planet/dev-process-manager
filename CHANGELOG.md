...
- 6da4629: Rename the package to `dev-process-manager` and require Node v22 or later
...
- 780e20e: Add "st" alias for status command
- 1a5cc54: Internal: the daemon now sends status as structured JSON and the table rendering is done in the CLI client (no change to the `status` output)
