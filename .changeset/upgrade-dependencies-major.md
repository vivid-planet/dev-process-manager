...
-   Tooling bumped to new majors: `@comet/eslint-config` 9, `npm-run-all2` 9, `@types/node` 26.
...
`eslint` is kept at 9.x and `typescript` at 5.9.x: the current `@comet/eslint-config` (and the `typescript-eslint`/plugins it bundles) do not yet support `eslint` 10 or `typescript` 7, so bumping either breaks installation or linting.
